          ...
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td class="whitespace-no-wrap">
                ...
                <feather-icon icon="TrashIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2" @click.stop="eliminard(tr.idcategoria)" />
              </vs-td>
...
import DataViewSidebar from '@/views/registro/producto/categoria/Ncategoria.vue'
...
export default {
  ...
  computed: {
    ...
    errore(){
      return this.$store.state.dataCategoria.respuesta
    },
    queriedItems () {
  ...
  methods: {
    ...
    eliminard(id){
        axios.delete('/admin/categoria/eliminar', {params: { idcategoria: id }})
        .then((response) => {
    ...
    let abb = false;
    try {
      axios.put('/categoria/desactivar', parameters[0])
      .then((response) => {
          this.$store.dispatch('dataCategoria/listarCategoria')
    ...
    Activar1: function(parameters) {
      try {
        axios.put('/categoria/activar', parameters[0])
        .then((response) => {
          this.$store.dispatch('dataCategoria/listarCategoria')
