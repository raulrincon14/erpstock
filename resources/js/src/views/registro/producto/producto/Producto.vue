...
<template>
  <div id="data-list-list-view" class="data-list-container">
    ...
    <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="products">
      ...
      <template slot="thead">
        <vs-th vs-justify="center" sort-key="name">Imagen</vs-th>
        <vs-th sort-key="category">Nombre</vs-th>
        <vs-th sort-key="category">Medida</vs-th>
        <vs-th sort-key="category">Precio C.</vs-th>
        <vs-th sort-key="category">Precio V.</vs-th>
        <vs-th sort-key="category">Codigo de Barra</vs-th>
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              <vs-td vs-justify="center" class="img-container">
                 <!-- <img :src = "'/storage/productos/'+tr.pr_codi+'.png'" @ error = "'/storage/productos/img.png'" width="110" class="product-img"> -->
                 <img :src="'/storage/productos/'+tr.pr_codi+'.png?timestamp=' + new Date().getTime()" @error="$event.target.src = '/storage/productos/img.png'" class="product-img" height="110" />
              </vs-td>
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.pr_nombre }}</p>
              </vs-td>
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.pr_medida +' '+tr.u_abre }}</p>
              </vs-td>
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.pr_precioc }}</p>
              </vs-td>
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.pr_preciov }}</p>
              </vs-td>
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.pr_cb }}</p>
              </vs-td>
              ...
              <vs-td class="whitespace-no-wrap">
                ...
                <feather-icon icon="EyeIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2" @click.stop="eliminard(tr.idmarca)" />
              </vs-td>
...
import DataViewSidebar from '@/views/registro/producto/producto/Nproducto.vue'
import moduleProducto from '@/store/registro/producto/moduleProducto.js'
...
export default {
  ...
  computed: {
    ...
    products () {
      return this.$store.state.dataProducto.productos
  ...
  methods: {
    ...
    async eliminar(id) {
      ...
      try {
            await this.$store.dispatch('dataProducto/eliminarProducto', id)
              if (this.errore) {
      ...
      // this.$store.dispatch('dataCategoria/eliminarCategoria', id)
      // await this.$store.dispatch('dataCategoria/eliminarCategoria', id)
      // await this.$store.dispatch('dataCategoria/eliminarCategoria', id)
      //   if (this.respuesta) {
      //     await this.notificacion('Se elimnó con éxito','success','check_circle');
      //   }
    ...
    eliminard(id){
        axios.delete('/admin/producto/eliminar', {params: { idproducto: id }})
        .then((response) => {
          if (response.data.res == "si") {
            this.$store.dispatch('dataProducto/listarProducto')
    ...
    let abb = false;
    try {
      axios.put('/producto/desactivar', parameters[0])
      .then((response) => {
          this.$store.dispatch('dataProducto/listarProducto')
    ...
    Activar1: function(parameters) {
      try {
        axios.put('/producto/activar', parameters[0])
        .then((response) => {
          this.$store.dispatch('dataProducto/listarProducto')
  ...
  created () {
    if (!moduleProducto.isRegistered) {
      this.$store.registerModule('dataProducto', moduleProducto)
      moduleProducto.isRegistered = true
    }
    this.$store.dispatch('dataProducto/listarProducto')
  },
  mounted () {
