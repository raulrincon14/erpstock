...
<template>
  ...
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    <div class="mt-6 flex items-center justify-between px-6">
        <h4>{{ Object.entries(this.data).length === 0 ? "Nuevo" : "Actualizar" }} Tipo</h4>
  ...
  watch: {
    isSidebarActive (val) {
      ...
      if (Object.entries(this.data).length === 0) {
        ...
        const { ti_nombre, ti_desc } = JSON.parse(JSON.stringify(this.data))
        this.dataName = ti_nombre
        this.desc = ti_desc
        this.initValues()
        console.log(this.data);
        console.log('estamos en el else' + this.dataName);
      }
      // Object.entries(this.data).length === 0 ? this.initValues() : { this.dataId, this.dataName, this.dataCategory, this.dataOrder_status, this.dataPrice } = JSON.parse(JSON.stringify(this.data))
    }
  },
  computed: {
    ...mapState('tipo', ['respuesta'])
    ,
    repito(){
      return this.$store.state.dataTipo.respuesta
    },
      ...
      if (result) {
          try {
              const obj = {
                  ...
                  ti_nombre: this.dataName,
                  ti_desc: this.desc
              }
              if (this.data.idtipo !== null && this.data.idtipo >= 0) {
                  await this.$store.dispatch('dataTipo/updateTipo', obj)
              } else {
                await this.$store.dispatch('dataTipo/addTipo', obj)
                  if (this.respuesta) {
