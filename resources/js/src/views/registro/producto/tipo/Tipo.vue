        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.ti_nombre }}</p>
              </vs-td>

              <vs-td>
                <p class="product-category">{{ tr.ti_desc }}</p>
              </vs-td>
              <vs-td>
                <div v-if="tr.ti_estado === 1">
              ...
              <vs-td class="whitespace-no-wrap">
                ...
                <feather-icon icon="TrashIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2" @click.stop="eliminard(tr.idtipo)" />
              </vs-td>
...
import DataViewSidebar from '@/views/registro/producto/tipo/Ntipo.vue'
import moduleTipo from '@/store/registro/tipo/moduleTipo.js'
...
export default {
  ...
  computed: {
    ...
    products () {
      return this.$store.state.dataTipo.tipos
  ...
  methods: {
    ...
    async eliminar(id) {
      ...
      try {
            await this.$store.dispatch('dataTipo/eliminarTipo', id)
              if (this.errore) {
    ...
    eliminard(id){
        axios.delete('/admin/tipo/eliminar', {params: { idtipo: id }})
        .then((response) => {
          if (response.data.res == "si") {
            this.$store.dispatch('dataTipo/listarTipo')
    ...
    let abb = false;
    try {
      axios.put('/tipo/desactivar', parameters[0])
      .then((response) => {
          this.$store.dispatch('dataTipo/listarTipo')
    ...
    Activar1: function(parameters) {
      try {
        axios.put('/tipo/activar', parameters[0])
        .then((response) => {
          this.$store.dispatch('dataTipo/listarTipo')
  ...
  created () {
    if (!moduleTipo.isRegistered) {
      this.$store.registerModule('dataTipo', moduleTipo)
      moduleTipo.isRegistered = true
    }
    this.$store.dispatch('dataTipo/listarTipo')
    console.log(this.$store.dispatch('dataTipo/listarTipo'));console.log(this.products);
  },
  mounted () {
    this.isMounted = true
  }
