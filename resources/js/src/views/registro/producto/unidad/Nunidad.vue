  ...
  watch: {
    isSidebarActive (val) {
      ...
      if (Object.entries(this.data).length === 0) {
        ...
        const { u_nombre, u_abre } = JSON.parse(JSON.stringify(this.data))
        this.dataName = u_nombre
        this.desc = u_abre
        this.initValues()
      }
      ...
      if (result) {
          try {
              const obj = {
                  ...
                  u_nombre: this.dataName,
                  u_abre: this.desc
              }
              if (this.data.idunidad !== null && this.data.idunidad >= 0) {
                  await this.$store.dispatch('dataUnidad/updateUnidad', obj)
              } else {
                await this.$store.dispatch('dataUnidad/addUnidad', obj)
                  if (this.respuesta) {
