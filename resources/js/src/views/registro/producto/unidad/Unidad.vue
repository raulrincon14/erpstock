  ...
  <div id="data-list-list-view" class="data-list-container">
    ...
    <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="products">
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="category">Abreviatura</vs-th>
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.u_nombre }}</p>
              </vs-td>

              <vs-td>
                <p class="product-category">{{ tr.u_abre }}</p>
              </vs-td>
              <vs-td>
                <div v-if="tr.u_estado === 1">
              ...
              <vs-td class="whitespace-no-wrap">
                ...
                <feather-icon icon="TrashIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2" @click.stop="eliminard(tr.idunidad)" />
              </vs-td>
...
import DataViewSidebar from '@/views/registro/producto/unidad/Nunidad.vue'
import moduleUnidad from '@/store/registro/unidad/moduleUnidad.js'
...
export default {
  ...
  computed: {
    ...
    products () {
      return this.$store.state.dataUnidad.unidades
  ...
  methods: {
    ...
    async eliminar(id) {
      ...
      try {
            await this.$store.dispatch('dataUnidad/eliminarUnidad', id)
              if (this.errore) {
    ...
    eliminard(id){
        axios.delete('/admin/unidad/eliminar', {params: { idunidad: id }})
        .then((response) => {
          if (response.data.res == "si") {
            this.$store.dispatch('dataUnidad/listarUnidad')
    ...
    let abb = false;
    try {
      axios.put('/unidad/desactivar', parameters[0])
      .then((response) => {
          this.$store.dispatch('dataUnidad/listarUnidad')
    ...
    Activar1: function(parameters) {
      try {
        axios.put('/unidad/activar', parameters[0])
        .then((response) => {
          this.$store.dispatch('dataUnidad/listarUnidad')
  ...
  created () {
    if (!moduleUnidad.isRegistered) {
      this.$store.registerModule('dataUnidad', moduleUnidad)
      moduleUnidad.isRegistered = true
    }
    this.$store.dispatch('dataUnidad/listarUnidad')
    console.log(this.$store.dispatch('dataUnidad/listarUnidad'));console.log(this.products);
  },
  mounted () {
    this.isMounted = true
  }
