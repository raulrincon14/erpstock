...
<template>
  ...
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    <div class="mt-6 flex items-center justify-between px-6">
        <h4>{{ Object.entries(this.data).length === 0 ? "Nuevo" : "Actualizar" }} Proveedor</h4>
...
export default {
  ...
  watch: {
    isSidebarActive (val) {
      ...
      if (Object.entries(this.data).length === 0) {
        ...
        const { pro_doc, pro_nombre, pro_docnum, pro_razon, pro_pais, pro_departamento, pro_provincia, pro_distrito, pro_direccion, pro_telefono, pro_email, pro_web} = JSON.parse(JSON.stringify(this.data))
        this.nombre = pro_nombre
        this.sdoc = pro_doc
        this.ncomercial = pro_razon
        this.ndoc = pro_docnum
        this.spaises = pro_pais
        this.sdepartamento = pro_departamento
        if (this.sdepartamento.iddepartamento) {
          this.llenarDepartamento();
        }
        this.sprovincia = pro_provincia
        if (this.sprovincia.idprovincia) {
          this.llenarProvincia(this.sdepartamento.iddepartamento);
        }
        this.sdistrito = pro_distrito
        if (this.sdistrito.iddistrito) {
          this.llenarDistrito(this.sprovincia.idprovincia);
        }
        this.direc = pro_direccion
        this.telefono = pro_telefono
  ...
  methods: {
    ...
    initValues () {
      if (this.data.idproveedor) return
      this.nombre = ''
      this.ndoc = ''
      this.ncomercial = ''
      this.spaises = ''
      ...
      if (result) {
          try {
              const obj = {
                  ...
                  pro_doc: this.sdoc,
                  pro_docnum: this.ndoc,
                  pro_nombre: this.nombre,
                  pro_razon: this.ncomercial,
                  pro_pais: this.spaises,
                  pro_departamento: this.sdepartamento,
                  pro_provincia: this.sprovincia,
                  pro_distrito: this.sdistrito,
                  pro_direccion: this.direc,
                  pro_telefono: this.telefono,
                  pro_email: this.email,
                  pro_web: this.web
              }
              if (this.data.idproveedor !== null && this.data.idproveedor >= 0) {
                  await this.$store.dispatch('dataProveedor/updateProveedor', obj)
              } else {
                await this.$store.dispatch('dataProveedor/addProveedor', obj)
                  if (this.respuesta) {
