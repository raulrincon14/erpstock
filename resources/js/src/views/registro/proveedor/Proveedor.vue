  ...
  <div id="data-list-list-view" class="data-list-container">
    ...
    <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="products">
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="category">Número</vs-th>
        <vs-th sort-key="category">Proveedor</vs-th>
        <vs-th sort-key="category">Telefono</vs-th>
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              <vs-td>
                <div v-if="tr.pro_doc.value == '1'">
                  <p class="product-name font-medium truncate">Sin Doc</p>
                </div>
                <div v-else-if="tr.pro_doc.value == '2'">
                    <p class="product-name font-medium truncate">DNI</p>
                  </div>
                <div v-else-if="tr.pro_doc.value == '3'">
                      <p class="product-name font-medium truncate">RUC</p>
                    </div>
                    <div v-else>
                      <p class="product-name font-medium truncate">Pasaporte</p>
                      </div>
              </vs-td>
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.pro_docnum}}</p>
              </vs-td>
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.pro_nombre }}</p>
              </vs-td>
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.pro_telefono }}</p>
              </vs-td>
              <vs-td>
                <div v-if="tr.pro_estado === 1">
              ...
              <vs-td class="whitespace-no-wrap">
                ...
                <feather-icon icon="EyeIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2" @click.stop="eliminard(tr.idproveedor)" />
              </vs-td>
...
import DataViewSidebar from '@/views/registro/proveedor/Nproveedor.vue'
import moduleProveedor from '@/store/registro/proveedor/moduleProveedor.js'
...
export default {
  ...
  computed: {
    ...
    products () {
      return this.$store.state.dataProveedor.proveedores
  ...
  methods: {
    ...
    let abb = false;
    try {
      axios.put('/proveedor/desactivar', parameters[0])
      .then((response) => {
          this.$store.dispatch('dataProveedor/listarProveedor')
    ...
    Activar1: function(parameters) {
      try {
        axios.put('/proveedor/activar', parameters[0])
        .then((response) => {
          this.$store.dispatch('dataProveedor/listarProveedor')
  ...
  created () {
    if (!moduleProveedor.isRegistered) {
      this.$store.registerModule('dataProveedor', moduleProveedor)
      moduleProveedor.isRegistered = true
    }
    this.$store.dispatch('dataProveedor/listarProveedor')
  },
  mounted () {
