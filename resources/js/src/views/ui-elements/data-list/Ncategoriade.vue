...
export default {
  ...
  methods: {
    initValues () {
      if (this.data.idcategoria) return
      this.dataName = ''
      this.desc = ''
    },
    async submitData (){
      ...
      if (result) {
          try {
              ...
              if (this.data.idcategoria !== null && this.data.idcategoria >= 0) {
                  await this.$store.dispatch('dataCategoria/updateCategoria', obj)
              } else {
                await this.$store.dispatch('dataCategoria/addCategoria', obj)
                  if (this.respuesta) {
